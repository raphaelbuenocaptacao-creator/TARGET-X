// TARGET X — Explorador de Renda + Universo Oficial 3.601 — v5.6
(function(){
  if(window.__txProfileExplorerV56)return; window.__txProfileExplorerV56=true;
  ...
  const parseMoney=v=>{if(typeof v==='number')return v;let s=String(v??'').replace(/R\$|\s/g,'');if(!s)return 0;if(s.includes(',')&&s.includes('.'))s=s.replace(/\./g,'').replace(',','.');else if(s.includes(','))s=s.replace(',','.');return Number(s)||0};
  const officialTotal=()=>Number(window.DATA_META?.records)||3601;
  let core=[],coreMeta=null,loading=false;

  const css=document.createElement('style');
  css.textContent=`
  .incomeLab{margin-top:12px;border:1px solid #315c47;background:linear-gradient(180deg,#0b1b13,#07100c);border-radius:18px;padding:15px}.incomeLabTop{display:flex;justify-content:space-between;align-items:flex-end;gap:12px;flex-wrap:wrap}.incomeLabTitle{font-size:16px;font-weight:1000;text-transform:uppercase;letter-spacing:1px}.incomeLabSub{font-size:11px;color:var(--muted);margin-top:4px}.incomeControls{display:grid;grid-template-columns:minmax(180px,1fr) minmax(170px,.7fr) auto;gap:8px;margin-top:12px}.incomeQuick{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.incomeQuick button{padding:7px 9px;font-size:10px}.incomeResult{display:none;margin-top:13px}.incomeResult.show{display:block}.incomeHero{display:grid;grid-template-columns:1.1fr .9fr;gap:10px}.incomeScore{font-size:30px;font-weight:1000;margin:6px 0}.incomeScore.good{color:var(--green)}.incomeScore.risk{color:var(--red)}.incomeScore.mid{color:var(--amber)}.incomeStats{display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));gap:8px;margin-top:10px}.incomeStat{border:1px solid var(--line);background:#06100b;border-radius:13px;padding:11px}.incomeStat span{display:block;color:var(--muted);font-size:9px;text-transform:uppercase;letter-spacing:1px}.incomeStat b{display:block;font-size:19px;margin-top:5px}.incomeProfile{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px}.incomeProfile .insight{min-height:75px}.incomeNear{overflow:auto;margin-top:10px}.incomeNear table{width:100%;border-collapse:collapse;min-width:620px}.incomeNear th,.incomeNear td{padding:7px;border-bottom:1px solid var(--line);text-align:right}.incomeNear th:first-child,.incomeNear td:first-child{text-align:left}.incomeNear th{font-size:9px;color:var(--muted);text-transform:uppercase}.incomeCoverage{font-size:10px;color:var(--muted);margin-top:8px}.officialUniverse{color:var(--green);font-weight:900}
  @media(max-width:900px){.incomeHero,.incomeProfile{grid-template-columns:1fr}.incomeStats{grid-template-columns:1fr 1fr}}@media(max-width:560px){.incomeControls{grid-template-columns:1fr}.incomeStats{grid-template-columns:1fr 1fr}}
  ...
  function parseCore(payload){
    ...
    return payload.r.map(a=>{
      ...
      return {date:String(a[0]||''),q:Q[a[6]]||'',grossVgv:Number(a[5])||0,contracts,activeVgv:Number(a[8])||0,activeContracts,cancelContracts,buyer:activeContracts>0,cancelled:cancelContracts>0,city:CITY[a[12]]||'',profession:PROF[a[13]]||'',income:parseMoney(INC[a[14]]||''),age:Number(a[15])||0};
    }).filter(r=>/^2026-\d{2}-\d{2}$/.test(r.date));
  }

  async function loadCore(){
    if(core.length||loading)return;loading=true;
    try{const r=await fetch(`${CORE_URL}?v=20260908-profile56`,{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);const p=await r.json();core=parseCore(p);coreMeta={from:p.from||'',to:p.to||'',rows:core.length};}
  ...
  function officialForFilters(){const scope=$('cpScope')?.value||'all';if(scope!=='all')return null;const p=$('cpPeriod')?.value||'all',q=$('cpQual')?.value||'all';const months=Array.isArray(window.MONTHS)?window.MONTHS:[];if(p==='all'){if(q==='Q')return months.reduce((s,m)=>s+(Number(m.q)||0),0);if(q==='NQ')return months.reduce((s,m)=>s+(Number(m.nq)||0),0);return officialTotal()}const m=months.find(x=>Number(x.month)===Number(p));if(!m)return null;if(q==='Q')return Number(m.q)||0;if(q==='NQ')return Number(m.nq)||0;return Number(m.couples)||0}

  function patchOfficial(){
    const off=officialForFilters();if(!off)return;
    const stats=$('cpStats');if(stats){const card=[...stats.querySelectorAll('.stat')].find(c=>/CASAIS ATENDIDOS/i.test(c.querySelector('.label')?.textContent||''));if(card){const b=card.querySelector('b'),sm=card.querySelector('small');if(b)b.textContent=num(off);if(sm)sm.innerHTML=`<span class="officialUniverse">universo oficial</span> • ${num(core.length||0)} registros na base analítica`}}
    const range=$('cpRange');if(range)range.textContent=`${num(off)} casais no universo oficial • análise detalhada conforme cobertura dos campos`;
    const warn=$('cpBaseWarning');if(warn){const to=coreMeta?.to?coreMeta.to.split('-').reverse().join('/'):'—';warn.innerHTML=`<b>Universo oficial:</b> ${num(off)} casais ${($('cpPeriod')?.value||'all')==='all'?'até 07/09/2026':'no período selecionado'}. <b>Base analítica:</b> ${num(core.length)} registros disponíveis até ${to}. O TARGET X mostra a cobertura real de cada campo e não inventa dados ausentes.`}
  }

  function mount(){
    const page=$('clientprofile');if(!page||$('incomeLab'))return;
    const filterCard=page.querySelector('.card');if(!filterCard)return;
    const box=document.createElement('div');box.id='incomeLab';box.className='incomeLab';box.innerHTML=`
    ...
    $('incomeAnalyze').onclick=analyzeIncome;$('incomeValue').addEventListener('keydown',e=>{if(e.key==='Enter')analyzeIncome()});box.querySelectorAll('[data-inc]').forEach(b=>b.onclick=()=>{$('incomeValue').value=b.dataset.inc;analyzeIncome()});
    ...
    const stats=$('cpStats');if(stats&&!stats.dataset.officialWatch){stats.dataset.officialWatch='1';new MutationObserver(()=>patchOfficial()).observe(stats,{childList:true,subtree:true,characterData:true})}
    patchOfficial();
  }

  function analyzeIncome(){
    const value=parseMoney($('incomeValue')?.value);const result=$('incomeResult');if(!result)return;if(!value){result.classList.add('show');result.innerHTML='<div class="cpWarn">Digite uma renda válida, por exemplo <b>20000</b>.</div>';return}if(!core.length){result.classList.add('show');result.innerHTML='<div class="cpWarn">A base de renda ainda está carregando. Tente novamente em alguns segundos.</div>';loadCore();return}
    ...
    const rich=richRows().filter(r=>{const inc=Number(r.income)||0,period=$('cpPeriod')?.value||'all',q=$('cpQual')?.value||'all';return inc>=rule.min&&inc<=rule.max&&(period==='all'||Number(String(r.date||'').slice(5,7))===Number(period))&&(q==='all'||r.qual===q)}),[car,carN]=top(rich,'car'),[rel,relN]=top(rich,'rel'),homeKnown=rich.filter(r=>r.home&&r.home!=='NÃO INFORMADO'),own=homeKnown.filter(r=>r.home==='CASA PRÓPRIA').length;
    const convDelta=(A.conv-B.conv)*100,cancelDelta=(A.cancelRate-B.cancelRate)*100;let grade='Perfil intermediário',klass='mid',read='Performance próxima da média da base conhecida.';if(A.conv>=B.conv+.02&&A.cancelRate<=B.cancelRate+.01){grade='Perfil forte',klass='good';read='Converte acima da média sem elevar o risco de cancelamento.'}if(A.cancelRate>B.cancelRate+.05){grade='Perfil de atenção',klass='risk';read='Apresenta cancelamento acima da média e merece abordagem/qualificação mais cuidadosa.'}
    const universe=officialForFilters()||officialTotal(),nearGroups={};known.filter(r=>Math.abs(r.income-value)<=5000).forEach(r=>{const k=Math.round(r.income);(nearGroups[k]||(nearGroups[k]=[])).push(r)});const near=Object.entries(nearGroups).map(([k,v])=>[Number(k),agg(v)]).filter(([,a])=>a.n>=3).sort((a,b)=>a[0]-b[0]).slice(0,15);
    const proflist=topList(seg,'profession',5),citylist=topList(seg,'city',5);
    result.classList.add('show');result.innerHTML=`
  ...
  function boot(){let tries=0;const t=setInterval(()=>{tries++;if($('clientprofile')){clearInterval(t);mount();loadCore();setTimeout(patchOfficial,500)}else if(tries>30)clearInterval(t)},200);document.addEventListener('click',e=>{if(e.target?.dataset?.page==='clientprofile')setTimeout(()=>{mount();patchOfficial()},100)})}
  boot();
})();