...
// ---------------- CAPITÃO DA SEMANA + LANÇAMENTO RÁPIDO ----------------
(function(){
  ...
  if(!$('captain')){
    ...
    page.className='page';page.id='captain';
    page.innerHTML=`<div class="section"><div><h2>Capitão da Semana</h2><div class="meta">Ciclo fixo de segunda a domingo • maior número de casais</div></div></div><div class="weekNav card"><button id="weekPrev">← Semana anterior</button><input id="weekAnchor" type="date"><button id="weekNow" class="primary">Semana atual</button><button id="weekNext">Próxima semana →</button><span class="meta" id="weekRange"></span></div><div class="captainHero" style="margin-top:11px"><div class="card"><div class="label" id="captainStatus">Líder provisório</div><div class="captainName" id="captainName">—</div><b id="captainResult">0 casais</b><div class="meta" style="margin-top:8px">Desempate por vendas.</div></div><div class="card"><div class="label">Premiação</div><b style="display:block;font-size:27px;margin:9px 0">R$ 200</b><div>Voucher</div><div class="green" style="font-weight:900;margin-top:9px">✓ Livre na semana</div></div></div><div class="section"><div><h2>Lançamento rápido de casais</h2><div class="meta">Para completar o resultado do dia sem preencher os outros indicadores.</div></div></div><div class="card"><div class="quick3"><input id="quickDate" type="date" min="2026-09-06"><select id="quickPerson"></select><input id="quickCouples" type="number" min="1" placeholder="Qtd. casais"></div><div class="actions"><button class="primary" id="quickSave">Adicionar casais</button></div><div class="tiny" style="margin-top:8px">O lançamento entra automaticamente no mês, no ano e no ranking do Capitão da Semana.</div></div><div class="section"><h2>Ranking da semana</h2><span class="meta" id="weeklyTotal"></span></div><div class="card"><div class="tablewrap"><table class="table" style="min-width:650px"><thead><tr><th>#</th><th>Profissional</th><th>Equipe</th><th>Casais</th><th>Vendas</th></tr></thead><tbody id="weeklyBody"></tbody></table></div></div><div class="section"><h2>Lançamentos rápidos da semana</h2></div><div class="card"><div class="tablewrap"><table class="table" style="min-width:650px"><thead><tr><th>Data</th><th>Profissional</th><th>Casais</th><th></th></tr></thead><tbody id="quickBody"></tbody></table></div></div>`;
    const teams=$('teams');
  ...
  function weeklyRanking(w){const map={};weekEntries(w).forEach(x=>{if(!x.person)return;map[x.person]||(map[x.person]={name:x.person,couples:0,sales:0});map[x.person].couples+=Number(x.couples)||0;map[x.person].sales+=Number(x.sales)||0;});return Object.values(map).filter(x=>x.couples>0||x.sales>0).map(x=>{const p=effectivePerson(x.name);return {...x,group:p.group||'A DEFINIR'};}).sort((a,b)=>b.couples-a.couples||b.sales-a.sales||a.name.localeCompare(b.name));}
  function refreshQuickPeople(){const current=$('quickPerson')?.value;$('quickPerson').innerHTML=HIST.filter(x=>x.active).map(x=>effectivePerson(x.name)).sort((a,b)=>a.name.localeCompare(b.name)).map(x=>`<option>${x.name}</option>`).join('');if(current&&[...$('quickPerson').options].some(o=>o.value===current))$('quickPerson').value=current;}
  function selectedWeek(){return weekBounds($('weekAnchor').value||iso(new Date()));}

  window.renderCaptain=function(){
    if(!$('captain'))return;
    if(!$('weekAnchor').value)$('weekAnchor').value=iso(new Date());
    if(!$('quickDate').value)$('quickDate').value=iso(new Date());
    refreshQuickPeople();
    ...
    $('captainResult').textContent=leader?`${num(leader.couples)} casais • ${num(leader.sales)} vendas`:'Nenhum casal lançado nesta semana';
    $('weeklyTotal').textContent=`${num(rank.reduce((a,x)=>a+x.couples,0))} casais lançados`;
    $('weeklyBody').innerHTML=rank.length?rank.map((x,i)=>`<tr><td><b>${i+1}º</b></td><td><span class="nameLink" data-week-person="${x.name.replace(/"/g,'&quot;')}">${x.name}</span></td><td>${x.group}</td><td><b>${num(x.couples)}</b></td><td>${num(x.sales)}</td></tr>`).join(''):'<tr><td colspan="5" class="empty">Ainda não há casais lançados para esta semana.</td></tr>';
  ...
  $('weekNow').onclick=()=>{$('weekAnchor').value=iso(new Date());renderCaptain();};
  $('quickSave').onclick=()=>{const date=$('quickDate').value,person=$('quickPerson').value,couples=Number($('quickCouples').value)||0;if(!date||date<'2026-09-06')return alert('Use uma data a partir de 06/09/2026.');if(!person||couples<=0)return alert('Escolha o profissional e informe a quantidade de casais.');launches.push({id:String(Date.now()),date,person,couples,sales:0,vgv:0,vgv_general:0,q:0,nq:0,notour:0,gift:0,cancelled:0,attendances:couples,source:'quick-couples'});saveState();$('quickCouples').value='';$('weekAnchor').value=date;renderCaptain();renderDashboard();renderRank();renderSeptember();renderLaunches();renderProfile();};
