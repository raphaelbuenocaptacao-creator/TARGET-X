...
(function(){
  ...
  function availableWeeks(){const set=new Set(Object.keys(WEEK_HISTORY));Object.keys(window.TX_CURRENT_WEEK_HISTORY||{}).forEach(k=>set.add(k));const rows=window.TX?.getRows?.()||[];rows.forEach(r=>r.date&&set.add(weekStartFor(r.date)));const through=window.TX?.getMeta?.().through||DATA_META.through;if(!set.size&&through)set.add(weekStartFor(through));return [...set].sort()}
  ...
  function rankingFor(start){const fresh=rankingFromRows(start),current=(window.TX_CURRENT_WEEK_HISTORY?.[start]||[]).map(x=>({name:x[0],couples:Number(x[1])||0,sales:Number(x[2])||0})),historic=(WEEK_HISTORY[start]||[]).map(x=>({name:x[0],couples:Number(x[1])||0,sales:Number(x[2])||0})),src=fresh.length?fresh:(current.length?current:historic);return src.filter(x=>!HIDDEN_PROFESSIONALS.has(x.name)&&effectivePerson(x.name).active).sort((a,b)=>b.couples-a.couples||b.sales-a.sales||a.name.localeCompare(b.name))}
